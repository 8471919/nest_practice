...
class MockUserRepository {
  #data = [{ id: 1, email: '8471919@naver.com' }];
  findOne({ where: { email } }) {
...
describe('UsersService', () => {
  ...
  it('findByEmail은 이메일을 통해 유저를 찾아야 함', () => {
    expect(service.findByEmail('8471919@naver.com')).resolves.toBe({
  ...
  it('findByEmail은 유저를 못 찾으면 null을 반환해야 함', () => {
    expect(service.findByEmail('7471919@naver.com')).resolves.toBe(null);
  });
});
